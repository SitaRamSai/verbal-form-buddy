...
const RULES: FieldRule[] = [
  ...
  {
    patterns: [/(?:social\s+security(?:\s+number)?|ssn)\s+(?:is\s+)?([\d\s-]+)/i],
    apply: (v) => {
  ...
  {
    patterns: [/((?:\d{1})\s*(?:foot|feet|ft)\s*(?:\d{1,2})?\s*(?:inch|inches|in)?)/i],
    apply: (v) => {
      ...
      return {
        ...
        ...(inches !== null && inches >= 0 && inches < 12 ? { heightInches: String(inches) } : {}),
      };
    },
  },
  {
  ...
  {
    patterns: [/(?:place\s+of\s+birth\s+is|i\s+was\s+born\s+in|born\s+in)\s+([^.,!?;]+)/i],
    apply: (v) => {
